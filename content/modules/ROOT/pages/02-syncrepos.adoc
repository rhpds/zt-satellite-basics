...
network. ##*Please do not synchronize the repositories*##. Please feel free to skip to the next assignment.

IMPORTANT: In order to speed up the lab, the RHEL 10 BaseOS and AppStream repos have been pre-synchronized. If you are familiar with this process please feel free to skip to the next assignment. If you decide to run through this procedure, please do not attempt to re-synchronize the repositories.

=====
.Red Hat Repositories - FOR DOCUMENTATION PURPOSES ONLY
[%collapsible]
====
...
lab we’ll synchronize the `+BaseOS+` and `+AppStream+` repos for RHEL 10.
...
=====
.Select repositories - FOR DOCUMENTATION PURPOSES ONLY
[%collapsible]
====
...
=====
.Initiate synchronization - FOR DOCUMENTATION PURPOSES ONLY
[%collapsible]
====
