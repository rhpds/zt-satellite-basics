...
In our registration command, we’ll specify that the new host should be added to the `+Application Servers+` host group we just created. As well, we’ll ignore certificate errors (since we’re using a self signed certificate) and we will set up Red Hat Lightspeed (formerly known as Red Hat Red Hat Insights).

== (Do not) Create a registration script

The following section is for documentation purposes. Please don't run the commands.

=====
.Create a registration script - DOCUMENTATION PURPOSES ONLY
[%collapsible]
====

WARNING: DO NOT RUN THIS COMMAND. IT IS FOR DOCUMENTATION PURPOSES ONLY.
...
WARNING: This command might take a few minutes to complete.

This script obtains the global registration script and runs it on
`+rhel1+` and `+rhel2+` via SSH. The global registration script contains operations to register the hosts to the Satellite server, install specified packages, configure Red Hat Lightspeed (formerly known as Red Hat Insights) and many more. You can find more information about the global registration script in the https://docs.redhat.com/en/documentation/red_hat_satellite/6.19/html/managing_hosts/registering-hosts-and-setting-up-host-integration_managing-hosts#Registering_Hosts_by_Using_Global_Registration_managing-hosts[Satellite documentation].
