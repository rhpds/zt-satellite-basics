...
`+Installable+` radio box.

.Installable radio box
image::installable-errata.png[installable radio box]
...
.Select all installable errata
image::errata-select-apply.png[select all installable errata]
...
In the sidebar menu click on `+Hosts+`, `+All Hosts+`, and then click on the host `+rhel1+`.
...
You can check on the status of the job by clicking the `+jobs+` button and then on the `+Resolve traces+` job.
